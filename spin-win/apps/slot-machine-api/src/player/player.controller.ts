import { Controller, Post, Body, Param, Get, Delete, HttpException, HttpStatus } from '@nestjs/common';
...
export class PlayerController {
  ...
  async createPlayer(@Body() data: CreatePlayerDto) {
    return await this.commandBus.execute(new CreatePlayerCommand(data.name, data.balance));
  }
  ...
  async depositMoney(@Param('id') id: string, @Body() data: DepositMoneyDto) {
    return await this.commandBus.execute(new DepositMoneyCommand(Number(id), data.amount));
  }
}
